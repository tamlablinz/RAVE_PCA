"""
RAVE Latent Space Analysis to PCA Coordinates (3D)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RAVE Latent Space Analysis to 3D PCA Coordinates and JSON Output"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of PCA components to compute (default: 3 for 3D output)"
    )
    parser.add_argument(
        "--sr",
        type=int,
    ...
    if args.n_components != 3:
        print("Warning: This version of the script is designed for 3 PCA components. "
              "The output will use generic keys for each component if n_components != 3.")
    ...
    # The JSON will have a top-level key "pca" mapping to a list of coordinate mappings.
    output_data = {"pca": []}
    ...
    for pca_coord, latent_coord in zip(latent_pca, latent_vectors):
        if args.n_components == 3:
            ...
            # Generic output for any number of components other than 3.
            coord_dict = {
